개인적인 학습을 위해 `이 곳`_ 에 있는 원본을 번역한 내용입니다. 오역이나 오류가 보이면 주저말고 Pull
...
URL 창 아래에 드롭다운으로 나타나죠. 대부분의 알고리즘은 결과를 검색 기록, 즐겨찾기, 쿠키, 그리고
...
라우터의 큐에 (네트워크 혼잡과 같은 이유로) 자리가 없을 때 패킷은 드롭됩니다.
...
* 서버는 요청을 다음의 파라미터들로 쪼개는데:
   * HTTP 요청 메소드 (``GET``, ``HEAD``, ``POST``, ``PUT``, ``PATCH``, ``DELETE``,
